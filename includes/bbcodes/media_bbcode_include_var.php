...
if (!empty($_GET['mediaSites'])) {
	function media_sites($sites) {
		$html = "";
		$last = array_keys($sites);
		$last = array_pop($last);
		foreach ($sites as $siteID => $link) {
			$html .= "<a href='$link' title='$link' target='_blank'>".ucfirst($siteID)."</a>".($last == $siteID ? "." : ", ");
		}
	...
	$__BBCODE__[] =
	array(
		"description" => 'Media BBCode',
		"value" => "!media",
		"bbcode_start" => "[media]",
		"bbcode_end" => "[/media]",
		"usage" => "[media]link[/media]",
		"html_start" => form_modal('media', 'Media', '', array(	'button_class' => 'bbcode',
		...
		"includejscript" => "media_bbcode_include_js.js",
		"html_end" => ""
	);
}
